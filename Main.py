...
de influencia del hecho''')
    ...
    DataLostchart = DataLost.groupby(
        ['MES'])[['Data perdida', 'Datos completos']].sum()
...
tres categórias, según su riesgo de accidente : Alto, Medio, Bajo''')
    ...
    st.markdown(
        '* **Alto ** Son los barrios que presentan un alto riesgo de accidentes, en el mapa se represetan con el color rosado')
    ...
    st.markdown(
        '* **Bajo** Son los barrios que presentan un  riesgo bajo de accidentes, en el mapa se represetan con el color Azul')
    st.text(
        'A continuación se presenta la agrupación de los barrios por su accidentalidad')
